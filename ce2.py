...
class CrossEncoder(PushToHubMixin):
    ...
    def predict(
        self,
        inp_dataloader: DataLoader | dict[str, torch.Tensor],
        batch_size: int = 32,
        ...
        # Handle both DataLoader and dict inputs
        if isinstance(inp_dataloader, DataLoader):
            iterator = inp_dataloader
            if show_progress_bar:
                iterator = tqdm(iterator, desc="Batches")
        else:
            # For dict input (CUDA graphs), use as single item
            iterator = [inp_dataloader]
